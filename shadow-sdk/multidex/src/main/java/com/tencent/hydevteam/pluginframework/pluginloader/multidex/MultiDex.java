...
public final class MultiDex {
    ...
    public static void install(ClassLoader loader, String apkPath, File secondaryDexDir, SharedPreferences prefs) {
        Log.i(TAG, "install");
        ...
        try {
            synchronized (installedApk) {
                ...
                File dexDir = getDexDir(secondaryDexDir);
                List<File> files = MultiDexExtractor.load(prefs, new File(apkPath), dexDir, false);
                if (checkValidZipFiles(files)) {
                    installSecondaryDexes(loader, dexDir, files);
                } else {
                    Log.w(TAG, "Files were not valid zip files.  Forcing a reload.");
                    // Try again, but this time force a reload of the zip file.
                    files = MultiDexExtractor.load(prefs, new File(apkPath), dexDir, true);
    ...
    private static File getDexDir(File secondaryDexDir)
            throws IOException {
        File cache = new File(secondaryDexDir, CODE_CACHE_NAME);
        try {
            mkdirChecked(cache);
        } catch (IOException e) {
             ...
             */
            cache = new File(secondaryDexDir, CODE_CACHE_NAME);
            mkdirChecked(cache);
        }
